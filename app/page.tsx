...
// ─── Loading Screen ───────────────────────────────────────────────────────────
// Drop Waqt.json into /public/waqt-animation.json for the real Lottie animation.
// Without lottie-web installed, the CSS-only fallback runs automatically.
...
function LoadingScreen({ onComplete, minDuration = 2800, isLoading = true }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => { if (!isLoading) setPhase("out"); }, minDuration);
    const t3 = setTimeout(() => { if (!isLoading) { setPhase("done"); onComplete?.(); } }, minDuration + 600);
  ...
  return (
    <>
      ...
      <div className={`wl-root wl-${phase}`} aria-label="Loading Waqt" role="status">
        <div className="wl-stars" aria-hidden="true">
          {WL_STARS.map((s, i) => (
            <div key={i} className="wl-star" style={{ left:s.x, top:s.y, animationDelay:s.d, animationDuration:s.dur }} />
          ))}
        </div>
        <div className="wl-crescent" aria-hidden="true">
          <svg viewBox="0 0 80 80" fill="none">
            <path d="M55 10 A30 30 0 1 0 55 70 A20 20 0 1 1 55 10Z" fill="rgba(255,199,0,0.18)" />
          </svg>
        </div>
        <div className="wl-card">
          <div className="wl-anim-wrap">
            {lottieLoaded ? (
              <div ref={lottieRef} className="wl-lottie" />
            ) : (
              <div className="wl-fallback" ref={lottieRef}>
                <svg className="wl-ring-svg" viewBox="0 0 345 345" fill="none">
                  <circle cx="172.5" cy="172.5" r="169" stroke="#07B061" strokeWidth="7" strokeLinecap="round" />
                </svg>
                <svg className="wl-arc-svg" viewBox="0 0 345 345" fill="none">
                  <circle className="wl-arc" cx="172.5" cy="172.5" r="169" stroke="#FFC700" strokeWidth="7" strokeLinecap="round" fill="none" />
                </svg>
                <div className="wl-mosque-wrap">
                  <svg className="wl-mosque" viewBox="0 0 240 200" fill="none">
                    <path d="M120 90 C80 90 55 115 55 150 L185 150 C185 115 160 90 120 90Z" fill="#e8f5e9"/>
                    <path d="M120 60 C100 60 85 75 85 90 L155 90 C155 75 140 60 120 60Z" fill="#e8f5e9"/>
                    <rect x="48" y="100" width="16" height="55" rx="2" fill="#e8f5e9"/>
                    <path d="M56 80 L48 100 L64 100Z" fill="#e8f5e9"/>
                    <rect x="53" y="72" width="6" height="10" rx="3" fill="#e8f5e9"/>
                    <rect x="176" y="100" width="16" height="55" rx="2" fill="#e8f5e9"/>
                    <path d="M184 80 L176 100 L192 100Z" fill="#e8f5e9"/>
                    <rect x="181" y="72" width="6" height="10" rx="3" fill="#e8f5e9"/>
                    <path d="M108 120 C108 113 114 107 120 107 C126 107 132 113 132 120 L132 150 L108 150Z" fill="#1a6b3a" opacity="0.5"/>
                    <rect x="40" y="150" width="160" height="8" rx="2" fill="#e8f5e9"/>
                    <path d="M120 70 L122 76 L128 76 L123 80 L125 86 L120 82 L115 86 L117 80 L112 76 L118 76Z" fill="#FFC700" className="wl-star-spin"/>
                  </svg>
                </div>
              </div>
            )}
          ...
          <div className="wl-brand">
            <span className="wl-brand-ar">وقت</span>
            <span className="wl-brand-en">Waqt</span>
          </div>
          <p className="wl-tagline">ইসলামিক লাইফ কম্প্যানিয়ন</p>
          <div className="wl-bar-track">
            <div className="wl-bar-fill" />
            <div className="wl-bar-glow" />
          </div>
          <WlRotatingDua />
        </div>
        <div className="wl-footer">
          <span className="wl-dot" /><span>Built for the Modern Ummah</span><span className="wl-dot" />
        </div>
      </div>
    </>
  );
...
const WL_DUAS = [
  { ar:"بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ",            tr:"In the name of Allah, the Most Gracious" },
  { ar:"الْحَمْدُ لِلَّهِ رَبِّ الْعَالَمِينَ",              tr:"All praise is due to Allah, Lord of all worlds" },
  { ar:"سُبْحَانَ اللَّهِ وَبِحَمْدِهِ",                     tr:"Glory be to Allah and His is the praise" },
  { ar:"لَا إِلَهَ إِلَّا اللَّهُ مُحَمَّدٌ رَسُولُ اللَّهِ", tr:"There is no god but Allah, Muhammad is His messenger" },
];

function WlRotatingDua() {
  const [idx, setIdx]         = useState(0);
  const [visible, setVisible] = useState(true);
  useEffect(() => {
    const iv = setInterval(() => {
      setVisible(false);
      setTimeout(() => { setIdx(i => (i + 1) % WL_DUAS.length); setVisible(true); }, 400);
    }, 2200);
    return () => clearInterval(iv);
  }, []);
  const dua = WL_DUAS[idx];
  return (
    <div className={`wl-dua ${visible ? "wl-dua-in" : "wl-dua-out"}`}>
      <span className="wl-dua-ar">{dua.ar}</span>
      <span className="wl-dua-tr">{dua.tr}</span>
    </div>
  );
}

const WL_STARS = Array.from({ length: 30 }, (_, i) => ({
  x:   `${(i * 137.5) % 100}%`,
  y:   `${(i * 97.3)  % 100}%`,
  d:   `${(i * 0.23)  % 3}s`,
  dur: `${2 + (i * 0.4) % 3}s`,
...
const WL_STYLES = `
  ...
  .wl-root {
    ...
    background:radial-gradient(ellipse at 30% 20%,#0d3320 0%,#061a0e 45%,#020c06 100%);
    font-family:'Cormorant Garamond',Georgia,serif;
    overflow:hidden;transition:opacity .6s ease,transform .6s ease;
  }
  .wl-in   { opacity:0; transform:scale(1.04); }
  .wl-hold { opacity:1; transform:scale(1);    }
  .wl-out  { opacity:0; transform:scale(0.97); pointer-events:none; }
  .wl-root::before {
  ...
  .wl-stars { position:absolute;inset:0;pointer-events:none; }
  .wl-star {
    position:absolute;width:2px;height:2px;
    background:#fff;border-radius:50%;
    animation:wlTwinkle var(--dur,2s) ease-in-out infinite;opacity:.5;
  }
  @keyframes wlTwinkle { 0%,100%{opacity:.15;transform:scale(1)} 50%{opacity:.8;transform:scale(1.5)} }
  .wl-crescent {
    position:absolute;top:-40px;right:-30px;width:220px;height:220px;
    animation:wlFloat 6s ease-in-out infinite;pointer-events:none;
  }
  @keyframes wlFloat { 0%,100%{transform:translateY(0) rotate(-5deg)} 50%{transform:translateY(-12px) rotate(5deg)} }
  .wl-card {
    ...
    box-shadow:0 0 0 1px rgba(255,199,0,.06),0 40px 100px rgba(0,0,0,.6),inset 0 1px 0 rgba(255,255,255,.07);
    animation:wlCardIn .7s .2s cubic-bezier(.22,1,.36,1) both;
  }
  @keyframes wlCardIn { from{opacity:0;transform:translateY(32px) scale(.95)} to{opacity:1;transform:translateY(0) scale(1)} }
  .wl-anim-wrap { position:relative;width:200px;height:200px;display:flex;align-items:center;justify-content:center; }
  .wl-lottie { width:200px;height:200px; }
  .wl-fallback { position:relative;width:200px;height:200px;display:flex;align-items:center;justify-content:center; }
  .wl-ring-svg,.wl-arc-svg { position:absolute;inset:0;width:100%;height:100%; }
  .wl-arc {
    stroke-dasharray:1060;stroke-dashoffset:1060;
    animation:wlDraw 2s .4s cubic-bezier(.4,0,.2,1) forwards,wlGlow 1.5s 2.4s ease-in-out infinite;
    transform-origin:center;transform:rotate(-90deg);
  }
  @keyframes wlDraw  { to { stroke-dashoffset:0; } }
  @keyframes wlGlow  { 0%,100%{filter:drop-shadow(0 0 4px #FFC700)} 50%{filter:drop-shadow(0 0 14px #FFC700)} }
  .wl-mosque-wrap {
    position:absolute;inset:0;display:flex;align-items:center;justify-content:center;
    animation:wlMosqueIn .6s 1.8s cubic-bezier(.22,1,.36,1) both;
  }
  @keyframes wlMosqueIn { from{opacity:0;transform:scale(.8) translateY(8px)} to{opacity:1;transform:scale(1) translateY(0)} }
  .wl-mosque { width:130px;height:auto;filter:drop-shadow(0 0 12px rgba(26,107,58,.6)); }
  .wl-star-spin { animation:wlStarPulse 1.5s ease-in-out infinite;transform-origin:120px 78px; }
  @keyframes wlStarPulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.6;transform:scale(1.2)} }
  .wl-brand { display:flex;align-items:baseline;gap:10px;animation:wlFadeUp .5s .6s ease both; }
  .wl-brand-ar {
    font-family:'Amiri',serif;font-size:32px;font-weight:700;
    color:#FFC700;letter-spacing:.02em;text-shadow:0 0 24px rgba(255,199,0,.4);
  }
  .wl-brand-en {
    font-family:'Cormorant Garamond',serif;font-size:38px;font-weight:600;
    color:#e8f5e9;letter-spacing:.12em;text-transform:uppercase;
  }
  .wl-tagline {
    font-family:'Cormorant Garamond',serif;font-size:13px;font-weight:400;
    color:rgba(255,255,255,.45);letter-spacing:.25em;text-transform:uppercase;
    margin:-10px 0 0;animation:wlFadeUp .5s .75s ease both;
  }
  .wl-bar-track {
    position:relative;width:180px;height:2px;
    background:rgba(255,255,255,.08);border-radius:2px;overflow:visible;
    animation:wlFadeUp .5s .9s ease both;
  }
  .wl-bar-fill {
    height:100%;border-radius:2px;
    background:linear-gradient(90deg,#07B061,#FFC700);
    animation:wlBar 2.4s .5s cubic-bezier(.4,0,.2,1) forwards;width:0%;
  }
  @keyframes wlBar { 0%{width:0%} 30%{width:35%} 60%{width:62%} 85%{width:88%} 100%{width:100%} }
  .wl-bar-glow {
    position:absolute;top:-3px;right:0;width:20px;height:8px;
    background:radial-gradient(ellipse,rgba(255,199,0,.7) 0%,transparent 70%);
    animation:wlBarGlow 2.4s .5s cubic-bezier(.4,0,.2,1) forwards;
  }
  @keyframes wlBarGlow { 0%{right:100%;opacity:0} 10%{opacity:1} 85%{right:0%;opacity:1} 100%{right:0%;opacity:0} }
  .wl-dua {
    display:flex;flex-direction:column;align-items:center;gap:4px;
    min-height:52px;text-align:center;
    transition:opacity .35s ease,transform .35s ease;
  }
  .wl-dua-in  { opacity:1;transform:translateY(0); }
  .wl-dua-out { opacity:0;transform:translateY(6px); }
  .wl-dua-ar { font-family:'Amiri',serif;font-size:18px;color:rgba(255,255,255,.7);direction:rtl;line-height:1.6; }
  .wl-dua-tr { font-size:11px;color:rgba(255,255,255,.3);letter-spacing:.05em;font-style:italic; }
  .wl-footer {
    position:absolute;bottom:28px;
    display:flex;align-items:center;gap:10px;
    font-size:11px;color:rgba(255,255,255,.2);
    letter-spacing:.2em;text-transform:uppercase;
    animation:wlFadeUp .6s 1.2s ease both;z-index:2;
  }
  .wl-dot { width:3px;height:3px;border-radius:50%;background:rgba(255,199,0,.4); }
...
export default function Home() {
  ...
  return (
    ...
    <>
      ...
      {/* ══ LOADING SCREEN ══ */}
      <LoadingScreen onComplete={() => setAppReady(true)} minDuration={2800} isLoading={false} />

      <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily: lang==="en" ? "'Segoe UI','Helvetica Neue',sans-serif" : "'Hind Siliguri','Segoe UI',sans-serif", display:"flex", flexDirection:"column", transition:"background .25s,color .25s", overflowX:"hidden", visibility: appReady ? "visible" : "hidden" }}>
